...
def process(process_id, font_num_p_process, ttf_names, font_num, charset,
            fonts_file_path, sfd_path, charset_lenw, opts):
    for i in range(process_id * font_num_p_process,
                   (process_id + 1) * font_num_p_process):
        if i >= font_num:
            break
        ...
        for charid in range(len(charset)):
            ...
            txt_fpath = os.path.join(
                ...
                opts.split,
                fontname,
                fontname + '_' + '{num:0{width}}'.format(num=charid, width=charset_lenw) + '.txt'
            )
            try:
